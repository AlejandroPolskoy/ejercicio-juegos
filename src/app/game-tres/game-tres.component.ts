...
@Component({
  selector: 'app-game-tres',
  templateUrl: './game-tres.component.html',
  styleUrls: ['./game-tres.component.scss']
})
export class GameTresComponent implements OnInit {
  questionsType:any = [
    {
      title: "Gender",
      key: "gender",
  ...
  question(index:number, key:string, text:string, self:any) {
    if(self.target.className == "disabled" ) return;
    self.target.classList.add("disabled");
    this.clues++;
    
    for (const item of this.persons) {
      if(item[key] !== this.chosenPerson[key]) item.isDisabled = true;
    }
  }
  ...
  restart() {
    this.chosenPerson = this.persons[this.getRandom( this.persons.length )];
    this.clues = 0;
    let victims = this.elem.nativeElement.querySelectorAll('.disabled')
    for (const victim of victims) {
      victim.classList.toggle('disabled');
    }
    for (const item of this.persons) {
